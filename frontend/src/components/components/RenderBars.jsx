...
function RenderBars({
  ...
  return (
    <div className="relative flex flex-col justify-center items-center">
      <button
        className={`shadow mt-2 active:scale-90 shadow-gray-500 font-semibold text-green-700 hover:text-yellow-600 hover:shadow-gray-400 ${
          algorithm.length >= 2
            ? " md:mt-1 absolute top-[-30px] right-0 md:top-0 md:relative py-1 mt-1 md:px-4 md:py-2 px-2 text-[10px]"
            : "px-4 py-2"
        }  rounded-2xl cursor-pointer`}
        onClick={() => setViewMode(viewMode === "bars" ? "box" : "bars")}
      >
        {viewMode === "bars" ? "Box" : "Bars"}
      </button>
      ...
      {algorithmType === "searching" && isSearchComplete && (
        ...
        >
          {isFound ? (
            <div className="rounded-md p-1 shadow-sm shadow-green-500 text-green-500">
              {" "}
              <strong className="font-extrabold">{searchTarget}</strong> Found
              🎉😊
            </div>
          ) : (
            <div className="rounded-md p-1 shadow-sm shadow-rose-500 text-rose-500">
              {" "}
      ...
      <svg
        className={`p-2 w-full ${
          algorithm.length == 2 ? "flex-row h-[20vh]" : "flex-col"
        } ${
          raceMode && algorithm.length == 3
            ? "h-[15vh]  lg:h-[60vh] "
            : "h-[26vh] lg:h-[60vh]"
        } `}
        viewBox={`0 0 ${
          arr.length * (viewMode === "bars" ? barSpacing : boxSize + gap)
        } ${baseHeight + 50}`}
      >
        {current.array.map((value, index) => (
          <g key={index}>
            ...
            <text
              x={
                index * (viewMode === "bars" ? barSpacing : boxSize + gap) +
                (viewMode === "bars" ? barWidth / 2 : boxSize / 2)
              }
