            ...
            {solutionRun.length > 0 ? (
              ...
              <div className="flex w-full flex-col gap-2 p-1">
                <div className="flex justify-between my-1 w-full">
                  <div className="skeleton h-12 w-70 dark:bg-white/10 bg-black/10"></div>
                  <div className="skeleton h-12 w-65 dark:bg-white/10 bg-black/10"></div>
                </div>
                <div className="flex gap-4 mb-1">
                  <div className="skeleton h-10 w-25 dark:bg-white/10 bg-black/10"></div>
                  <div className="skeleton h-10 w-25 dark:bg-white/10 bg-black/10"></div>
                </div>
                <div className="skeleton h-7 w-20 dark:bg-white/10 bg-black/10"></div>
                <div className="skeleton h-14 w-full dark:bg-white/10 bg-black/10"></div>
                <div className="skeleton h-7 w-20 dark:bg-white/10 bg-black/10"></div>
                <div className="skeleton h-14 w-full dark:bg-white/10 bg-black/10 mb-5"></div>
              </div>
            ) : (
...
const Tabdiv = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`px-2 ${Cases && "px-3"
        } ml-1 py-1.5  flex m-1 items-center hover:bg-gray-100 dark:hover:bg-white/8 hover:rounded-lg gap-1 select-none ${active
          ? `text-black dark:text-white font-medium  ${ResultCases && "pl-0 pr-4"
          } ${(Cases || ResultCases) && "bg-black/5 dark:bg-white/10 rounded-lg"
          }`
          : `text-black/30 dark:text-white/30 cursor-pointer hover:text-black dark:hover:text-white ${ResultCases && "pl-0 pr-4"
          }`
        }`}
    >
